...
/*
  A 12-bit sampler to record sampled audio to SRAM, but playback is
  reversed.
...
void loop() {
  ...
  if ((mode == OFF) || (mode == PASSTHROUGH)) {
    ...
    if ((digitalRead(PLAY_BUTTON) == LOW) && (sampleRecorded)) {
      mode = PLAYBACK;
      address = endAddress - 3;
      addressChipNumber = endAddressChipNumber;
    }
  }
...
ISR(TIMER1_OVF_vect) {
  ...
  if (mode == PLAYBACK) {
    if (evenCycle) {
      ...
      if (address == 0) {
        if (addressChipNumber == 0) {
          // reached the beginning of SRAM 0.  Wrap around to end of sample.
          address = endAddress - 3;
          addressChipNumber = endAddressChipNumber;
        } else {
          // reached beginning of SRAM 1, now start at end of SRAM 0
          address = MAX_ADDR;
          addressChipNumber = 0;
        }
      } else {
